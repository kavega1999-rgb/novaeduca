...
const UserCalendar = () => {
  ...
  const fetchTrainings = async () => {
    ...
    // Get trainings assigned to user
    ...
    // Get user's area target trainings
    ...
    if (userArea) {
      const { data: areaTargets } = await supabase
        ...
        .select("training_id")
        .eq("target_area", userArea);
      areaTrainingIds = areaTargets?.map(a => a.training_id) || [];
    }

    // Get visible_to_all trainings + assigned + area-targeted
    const { data, error } = await supabase
      .from("trainings")
      .select("id, title, active_from, active_until, status, type, area_id, is_finished, visible_to_all, areas(name, color)")
    ...
    if (!error && data) {
      ...
      const filtered = data.filter(t => t.visible_to_all || allRelevantIds.has(t.id));
      setTrainings(filtered as any);
    }
    setLoading(false);
  };
      ...
      <div className="container mx-auto px-4 py-4 md:py-8 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Visualiza las fechas de tus capacitaciones asignadas.
          </p>
        </div>
